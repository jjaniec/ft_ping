...
static int			print_ignored_packet(char *reason, struct iphdr *ip_in, \
						struct icmphdr *icmp_in)
{
	if (g_ft_ping_info->opt.v)
	{
		printf("Ignored packet (%s) of %hu bytes" \
			" from %s: icmp_type=%"PRIu8" icmp_code=%"PRIu8" icmp.echo.id=%d sequence=%d ttl=%d\n", \
			reason, (uint16_t)(ntohs(ip_in->tot_len) - IPHDR_SIZE), \
			inet_ntoa((struct in_addr) {.s_addr = ip_in->saddr}), icmp_in->type, \
			icmp_in->code, htons(icmp_in->un.echo.id), icmp_in->un.echo.sequence, ip_in->ttl);
	}
	return (1);
}

static void			debug_ip_struct(struct iphdr *ip_in)
{
	printf("IPHDR STRUCT:\n{\n\tsaddr: %s\n", inet_ntoa((struct in_addr) {.s_addr = ip_in->saddr}));
	printf("\tdaddr: %s\n}\n", inet_ntoa((struct in_addr) {.s_addr = ip_in->daddr}));
	printf("\t%s\n",inet_ntoa(g_ft_ping_info->addr->sin_addr));
}

static void			debug_icmp_struct(struct icmphdr *icmp_in)
{
						...
						char *reply_data, struct timeval *stop)
{
	...
	// debug_ip_struct(ip_in);
	// debug_icmp_struct(icmp_in);
	if (icmp_in->un.echo.id != ntohs(getpid()) && icmp_in->un.echo.id != getpid())
		return (print_ignored_packet("pid != echo.id", ip_in, icmp_in));
	if (icmp_in->type == ICMP_ECHOREPLY)
	{
		...
		return (print_ignored_packet("icmp_type == ICMP_ECHO", ip_in, icmp_in));
	return (0);
}
